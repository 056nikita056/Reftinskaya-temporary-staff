...
export function Panel({ title, icon, children }: { title: string; icon?: ReactNode; children: ReactNode }) {
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-3">
      <div className="mb-3 flex items-center gap-2 font-black">{icon}{title}</div>
      {children}
    </section>
  );
}
...
export function SectionTitle({ title, count }: { title: string; count?: number }) {
  return <div className="my-3 rounded bg-slate-200 px-3 py-2 text-center text-sm font-black">{title} {count !== undefined && <span className="rounded-full bg-refGreen px-2 py-0.5 text-xs text-white">{count}</span>}</div>;
}

export function Readonly({ label, value }: { label: string; value: unknown }) {
  return <div className="rounded-md border border-slate-200 bg-white p-2"><p className="text-[11px] font-bold text-slate-500">{label}</p><p className="text-sm font-black">{String(value ?? "")}</p></div>;
}
...
export function Input({ label, value, onChange, ...inputProps }: { label: string; value: string | number; onChange: (value: string) => void } & Omit<InputHTMLAttributes<HTMLInputElement>, "value" | "onChange">) {
  return <label className="text-sm font-black">{label}<input {...inputProps} className="field mt-1" value={value} onChange={(e) => onChange(e.target.value)} /></label>;
}
  ...
  return (
    <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-600">
      <div className="mb-3 flex items-start gap-3">
        ...
        <div>
          <p className="font-black text-refDark">{title || text}</p>
          {title && <p className="mt-1 font-bold leading-relaxed text-slate-600">{text}</p>}
        </div>
      </div>
      {steps?.length ? (
        <div className="space-y-2 rounded-md bg-white p-3">
          {steps.map((step, index) => (
            <div key={step} className="flex gap-2 font-bold">
